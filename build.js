// build.js - AŞAMA 1 HATA DÜZELTME (TAM SÜRÜM)
...
function createPageTemplate(meta, mainContent, bodyClass = '') {
    ...
    // DÜZELTME: Sidebar HTML'i basitleştirildi, .sidebar-actions div'i kaldırıldı.
...
async function buildSite() {
    // Bu fonksiyonun geri kalanında hiçbir değişiklik yok, projedeki haliyle aynı kalabilir.
    // ... (build.js dosyasının geri kalanı) ...
    // ...
    ...
    if (await fs.pathExists(staticPagesDir)) {
        for (const pageFile of await fs.readdir(staticPagesDir)) {
            ...
            const meta = {
                'about': { title: 'Hakkında', description: 'Mustafa Günay kimdir? Bu blogun amacı ve hikayesi.', url: '/about.html' },
                'contact': { title: 'İletişim', description: 'Projeler, danışmanlık veya bir kahve eşliğinde teknoloji sohbeti için bana ulaşın.', url: '/contact.html' },
                'hizmetler': { title: 'Hizmetler', description: 'Siber güvenlik alanında sunduğum profesyonel hizmetler; sızma testi, danışmanlık ve eğitimler.', url: '/hizmetler.html' }
            }[pageName] || { title: pageName.charAt(0).toUpperCase() + pageName.slice(1), url: `/${pageFile}`};
