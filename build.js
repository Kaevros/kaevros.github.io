...
async function buildSite() {
    ...
    const indexContent = `<section class="hero-section" data-aos="fade-in"><p class="hero-subtitle">Teknolojiyi Anlamak, Güvenliği Sağlamak.</p></section><section class="latest-posts-section"><div class="styled-header"><h2 class="section-title" data-aos="fade-right" data-aos-anchor-placement="top-bottom">Son Keşifler</h2></div><div class="posts-grid">${allPosts.slice(0, 3).map((post, i) => createPostCard(post, i)).join('')}</div></section><section class="cta-section" data-aos="fade-up"><p>Daha derine inmeye hazır mısın?</p><div class="cta-buttons"><a href="/posts.html" class="cta-button">Tüm Yazıları Gör</a></div></section>`;
    await fs.writeFile(path.join(outputDir, 'index.html'), createPageTemplate('Mustafa Günay - Kişisel Blog', indexContent, 'home'));
    const postsPageContent = `<section class="content-page"><div class="styled-header"><h2 data-aos="fade-down" data-aos-anchor-placement="top-bottom">Tüm Yazılar</h2></div><div class="posts-grid">${allPosts.map((post, i) => createPostCard(post, i)).join('')}</div></section>`;
