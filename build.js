...
async function buildSite() {
    ...
    const rawImagesDir = path.join(__dirname, '_raw_assets', 'images');
    ...
    if (await fs.pathExists(rawImagesDir)) {
        ...
        if (imageFiles.length > 0) {
            console.log(`>>> ${imageFiles.length} adet ham görsel kontrol ediliyor...`);
    ...
    const indexContent = `<section class="hero-section" data-aos="fade-in"><p class="hero-subtitle animated-gradient-text orange-heavy">Teknolojiyi Anlamak, Güvenliği Sağlamak.</p></section><section class="latest-posts-section"><div class="styled-header"><h2 class="section-title" data-aos="fade-right">Son Keşifler</h2></div><div class="posts-grid">${allPosts.slice(0, 3).map((post, i) => createPostCard(post, i)).join('')}</div></section><section class="cta-section" data-aos="fade-up" data-aos-delay="200"><p>Daha derine inmeye hazır mısın?</p><div class="cta-buttons"><a href="/posts.html" class="cta-button">Tüm Yazıları Gör</a></div></section>`;
